...
#ifndef GNU_gama_gMatVec_MatSVD_h
#define GNU_gama_gMatVec_MatSVD_h
...
namespace GNU_gama {
  ...
  public:
    ...
    SVD(const Mat<Float, Index, Exc>& A)
      : m(A.rows()), n(A.cols()), U_(A),
    ...
    SVD& reset(const Mat<Float, Index, Exc>& A);
    SVD& reset(const Mat<Float, Index, Exc>& A,
    ...
    const Mat<Float, Index, Exc>& SVD_U() { svd(); return U_; }
    const Vec<Float, Index, Exc>& SVD_W() { svd(); return W_; }
    const Mat<Float, Index, Exc>& SVD_V() { svd(); return V_; }
  ...
  template <typename Float, typename Index, typename Exc>
  SVD<Float, Index, Exc>&
  SVD<Float, Index, Exc>::reset(const Mat<Float, Index, Exc>& A)
  {
  ...
  template <typename Float, typename Index, typename Exc>
  void SVD<Float, Index, Exc>::reset_UWV()
  {
    delete[] U;
    delete[] V;
  ...
  template <typename Float, typename Index, typename Exc>
  SVD<Float, Index, Exc>& SVD<Float, Index, Exc>::clear()
  {
    m = 0;
    n = 0;
    W_.reset();
    ...
    return *this;
  }      /* SVD& SVD<Float, Index, Exc>::clear() */


  template <typename Float, typename Index, typename Exc>
  Float SVD<Float, Index, Exc>::q_xx(Index i, Index j)
  {
    if (!(1 <= i && i <= n && 1 <= j && j <= n))
  ...
  template <typename Float, typename Index, typename Exc>
  Float SVD<Float, Index, Exc>::q_bb(Index i, Index j)
  {
    if (!(1 <= i && i <= m && 1 <= j && j <= m))
  ...
  template <typename Float, typename Index, typename Exc>
  Float SVD<Float, Index, Exc>::q_bx(Index i, Index j)
  {
    if (!(1 <= i && i <= m && 1 <= j && j <= n))
  ...
  template <typename Float, typename Index, typename Exc>
  void SVD<Float, Index, Exc>::min_x()
  {
  ...
  template <typename Float, typename Index, typename Exc>
  void SVD<Float, Index, Exc>::min_x(Index n, Index list[])
  {
    minx = subset;
  ...
  template <typename Float, typename Index, typename Exc>
  void SVD<Float, Index, Exc>::min_subset_x()
  {
    using namespace std;
  ...
  template <typename Float, typename Index, typename Exc>
  void SVD<Float, Index, Exc>::solve(const Vec<Float, Index, Exc>& rhs,
                                     Vec<Float, Index, Exc>& x_)
  {
    svd();
    ...
    // x = V*t;
    typename Vec<Float, Index, Exc>::iterator x = x_.begin();
